  ...
  test(
    ...
    async ({ page }) => {
      ...
      await test.step('Verify sort indicator is displayed', async () => {
        ...
        await expect(nameColumnHeader).toHaveAttribute('aria-sort', order);
      });
...
(['ascending', 'descending'] as const).forEach((order) => {
  test(
    'User can sort workspaces by Last modified column in ' + order + ' order',
    async ({ page }) => {
      /**
       * Jira References:
       * - https://issues.redhat.com/browse/RHINENG-22174– Sort workspaces by last modified
       * Metadata:
       * - requirements: inv-groups-get-by-id
       * - importance: high
       * - assignee: addubey
      ...
      const UNIQUE_UI_ID = 'sort_modified_' + order + '_' + Date.now();
      const workspaceNames = [
        UNIQUE_UI_ID + '_First',
        UNIQUE_UI_ID + '_Second',
        UNIQUE_UI_ID + '_Third',
        UNIQUE_UI_ID + '_Fourth',
        UNIQUE_UI_ID + '_Fifth',
      ];
      ...
      await test.step(
        'Sort by Last modified column in ' + order + ' order',
        async () => {
          const lastModifiedHeader = page.locator(
            'th[data-label="Last modified"] button, thead th:has-text("Last modified") button',
          );
          ...
          // Keep clicking until we reach the desired sort direction (max 3 clicks)
          for (let attempt = 0; attempt < 3; attempt++) {
            const currentSort = await lastModifiedHeader
              .locator('..')
              .getAttribute('aria-sort');
            ...
            await expect(async () => {
              const url = page.url();
              expect(url).toContain('order_by=updated');
            }).toPass({ timeout: 5000 });
          }

          // Verify we reached the target sort direction
          await expect(async () => {
            const finalSort = await lastModifiedHeader
              .locator('..')
              .getAttribute('aria-sort');
            expect(finalSort).toBe(order);
          }).toPass({ timeout: 5000 });
      ...
      await test.step('Verify sort indicator is displayed', async () => {
        const lastModifiedHeader = page.locator(
          'th[data-label="Last modified"], thead th:has-text("Last modified")',
        );
        await expect(lastModifiedHeader).toHaveAttribute('aria-sort', order);
      });

...
test.describe('Workspace CRUD - Details Page', () => {
  ...
  test('User cannot delete a workspace with systems from Workspace Details page', async ({
    ...
    await test.step('Search and open workspace with systems', async () => {
      ...
      await expect(searchInput).toBeVisible();
      await page.reload({ waitUntil: 'load' });
      await searchInput.fill(workspaceWithSystem.workspaceName);

      const workspaceLink = page.getByRole('link', {
        name: workspaceWithSystem.workspaceName,
      });
      await expect(workspaceLink).toBeVisible({ timeout: 100000 });
      await workspaceLink.click();
      await waitForWorkspaceDetailPageReady(page, {
...
test.describe('Workspace CRUD - List Page', () => {
...
test.describe('Workspace System Management', () => {
  test('User can add and remove system from workspace', async ({
    ...
    await test.step('Add system to workspace', async () => {
      const addSystemsButton = page.getByRole('button', {
        name: 'Add systems',
      });
    ...
    await test.step.skip('Remove system from workspace', async () => {
      ...
      const removeConfirmButton = dialog.getByRole('button', {
        name: 'Remove',
      });
...
test.describe('Workspace Navigation', () => {
  test('User can navigate to Workspace Info Tab', async ({ page }) => {
    ...
    await test.step('Verify Workspace info shows User access configuration section', async () => {
      const workspaceInfoTab = page.getByRole('tab', {
        name: 'Workspace info',
      });

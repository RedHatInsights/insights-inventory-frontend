...
export const TABLE_ROW_CHECKBOX =
...
export const CONDITIONAL_FILTER =
  '[data-ouia-component-id="ConditionalFilter"]';
export const CONDITIONAL_FILTER_TOGGLE =
  '[data-ouia-component-id="ConditionalFilterToggle"]';
export const PT_CONDITIONAL_FILTER =
  '[data-ouia-component-id="ConditionalFilter"]';
export const PT_CONDITIONAL_FILTER_TOGGLE =
  '[data-ouia-component-id="ConditionalFilterToggle"]';
export const PT_CONDITIONAL_FILTER_LIST =
  '[data-ouia-component-id="ConditionalFilterList"]';
export const PT_BULK_SELECT = '[data-ouia-component-id="BulkSelect"]';
export const PT_BULK_SELECT_CHECKBOX =
  '[data-ouia-component-id="BulkSelectCheckbox"]';
export const PT_BULK_SELECT_LIST = '[data-ouia-component-id="BulkSelectList"]';
export const PT_BULK_ACTIONS_TOGGLE =
  '[data-ouia-component-id="BulkActionsToggle"]';
export const PT_BULK_ACTIONS_LIST =
  '[data-ouia-component-id="BulkActionsList"]';

export const checkPaginationValues = (expectedValues) => {
  cy.get(PAGINATION_TOP).find(MENU_TOGGLE).click();
  cy.get(MENU)
    .find('.pf-v6-c-menu__list-item')
    .each(($el, index) => {
      cy.wrap($el).should('have.text', `${expectedValues[index]} per page`);
    });
};

export const changePagination = (paginationValue) => {
  cy.get(PAGINATION_TOP).find(MENU_TOGGLE).click();
  return cy.get(MENU).find(MENU_ITEM).contains(`${paginationValue}`).click();
};

export const hasChip = (name, value) => {
  cy.contains('.pf-v6-c-label-group', name);
  cy.contains(CHIP, value);
};

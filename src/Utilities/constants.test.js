...
import { reduceFilters, reloadWrapper } from './constants';
        ...
        tagFilters: [
          {
            key: 'something',
            values: [
              { key: 'test', tagKey: 'test', value: 'some', group: 'another' },
            ],
          },
        ],
      },
...
describe('generateFilter', () => {
  ...
  describe('filter by name or id', () => {
    ...
    it('should generate filter and rest empty - array', () => {
      const result = generateFilter(undefined, undefined, undefined, [
        'something',
      ]);
      expect(result.length).toBe(3);
      expect(result).toMatchObject([
